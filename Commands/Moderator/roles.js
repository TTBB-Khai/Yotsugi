...
TTBT.on("guildMemberAdd", (guild, member) => {
	...
	TTBT.createMessage(process.env['WELCOME_CHANNEL_ID'], `Welcome ${member.mention}, type '${process.env['JOIN_MESSAGE']}' to join.\nWARNING: You can only do this once!`);
});
...
TTBT.on('messageCreate', msg => {
	...
	} else if (msg.content !== process.env['JOIN_MESSAGE'] && msg.channel.id === process.env['WELCOME_CHANNEL_ID'] && msg.author.id !== process.env['CLIENT_ID']) {
